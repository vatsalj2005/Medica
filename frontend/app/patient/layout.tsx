...
const navLinks = [
  ...
  { href: "/patient/request-appointment", label: "Request Appointment", icon: "＋" },
...
export default function PatientLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const s = getSession();
    if (!s || s.role !== "patient") { router.replace("/login"); return; }
    setSession(s);
  }, [router]);
      ...
      <aside className="w-64 bg-[#1a1d27] border-r border-[#2a2d3a] flex flex-col">
        <div className="p-6 border-b border-[#2a2d3a]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-indigo-500/20 rounded-xl flex items-center justify-center text-lg">🏥</div>
            <div>
