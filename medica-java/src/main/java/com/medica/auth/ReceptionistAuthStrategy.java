...
/**
 * Concrete Strategy for receptionist authentication.
 * 
 * Design Pattern: Strategy (Behavioral)
 * Mirrors: lib/auth.ts — receptionist table lookup block
 */
public class ReceptionistAuthStrategy implements AuthStrategy {
    ...
    public AuthSession authenticate(String email, String password) throws SQLException {
        ...
        String sql = "SELECT r_id, name, email, password FROM receptionist WHERE email = ?";
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    if (BCrypt.checkpw(password, hashedPassword)) {
                        ...
                        return new AuthSession(UserRole.RECEPTIONIST, user);
                    }
                }
            }
        }
